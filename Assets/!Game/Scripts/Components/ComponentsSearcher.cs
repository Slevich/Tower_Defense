...
public static class ComponentsSearcher
{
    ...
    {
        Component searchingComponent = null;
        Component[] components = GetComponentsOfTypeFromObjectAndAllChildren(ParentComponentHolder, SearchingType);

        if (components == null || components.Length == 0)
            searchingComponent = null;
        else
            searchingComponent = components.First();
